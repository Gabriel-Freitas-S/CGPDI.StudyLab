...
Este roteiro organiza os tópicos práticos e teóricos necessários para obter **nota máxima (10.0)** nos três Trabalhos Computacionais do curso.

---

## 🎯 Trabalho T1 — Unidade 1: Introdução, Cor & PDI (Nota C1)

### Conteúdos Avaliados:
- Pipeline gráfico básico e manipulação de buffers de memória.
- Conversão de espaços de cor (RGB $\to$ HSV, YCbCr, Escala de Cinza ITU-R BT.709).
...
- Binarização de Otsu e Morfologia Matemática (Erosão/Dilatação).

### Roteiro Prático no CGPDI.StudyLab:
1. Abra a aba **🖼️ Processamento Digital de Imagens (PDI)**.
...
## 🎯 Trabalho T2 — Unidade 2: Geometria 3D, Malhas & Câmeras (Nota C2)

### Conteúdos Avaliados:
- Álgebra Linear 3D: Produto Escalar, Produto Vetorial e Matrizes de Rotação/Translação.
...
### Roteiro Prático no CGPDI.StudyLab:
1. Abra a aba **🧊 Computação Gráfica 3D (DirectX)** e teste a **Câmera Orbital Arcball** girando o cubo, a esfera e a pirâmide.
2. Alterne entre **Projeção Perspectiva** e **Projeção Ortográfica** e note como as linhas paralelas convergem para um ponto de fuga apenas na perspectiva.
...
## 🎯 Trabalho T3 — Unidade 3: Modelagem Hierárquica & Cinemática (Nota C3)
...
- Cinemática Direta (*Forward Kinematics*) aplicada a robôs ou esqueletos articulados.
- Animação em tempo real e reusabilidade de componentes gráficos.

### Roteiro Prático no CGPDI.StudyLab:
1. Abra a aba **🧊 Computação Gráfica 3D** $\to$ Seção **Modelagem Hierárquica**.
2. Carregue o **Braço Robótico Articulado** e mova individualmente os sliders de **Base**, **Ombro**, **Cotovelo** e **Pulso**.
3. Observe como a rotação da base move todos os membros seguintes juntos, mantendo as distâncias entre juntas perfeitamente preservadas.
